...
public class AuthServiceImpl implements AuthService {
    ...
    public ResponseEntity<JWTAuthResponse> signIn(SignIn signIn) {
        try {
            ...
            JWTAuthResponse response = new JWTAuthResponse(accessToken, refreshToken, "Login successful", 1);
            return ResponseEntity.ok(response);

        } catch (Exception e) {
            JWTAuthResponse response = new JWTAuthResponse(null, null, "Login failed: " + e.getMessage(), 0);
            return ResponseEntity.ok(response);
        }
    }
    ...
    public ResponseEntity<JWTAuthResponse> signUp(UserDto userDto) {
        try {
            ...
            if (existingUser.isPresent()) {
                return ResponseEntity.ok(new JWTAuthResponse(null, null, "User already exists", 0));
            }
            ...
            JWTAuthResponse response = new JWTAuthResponse(accessToken, refreshToken, "User registered successfully", 1);
            return ResponseEntity.ok(response);

        } catch (Exception e) {
            JWTAuthResponse response = new JWTAuthResponse(null, null, "Registration failed: " + e.getMessage(), 0);
            return ResponseEntity.ok(response);
        }
    }
